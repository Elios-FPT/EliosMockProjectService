...
{
    public class GetSubmissionsClasseaBySubmissionQuery : IQueryHandler<Contract.UseCases.Submission.Query.GetSubmissionsClasseaBySubmissionQuery, BaseResponseDto<List<SubmissionsClassDto>>>
    {
        private readonly IGenericRepository<Domain.Entities.SubmissionsClass> _classRepository;

        public GetSubmissionsClasseaBySubmissionQuery(IGenericRepository<Domain.Entities.SubmissionsClass> classRepository)
        {
            _classRepository = classRepository ?? throw new ArgumentNullException(nameof(classRepository));
        }

        public async Task<BaseResponseDto<List<SubmissionsClassDto>>> Handle(Contract.UseCases.Submission.Query.GetSubmissionsClasseaBySubmissionQuery request, CancellationToken cancellationToken)
        {
            try
            {
                var classes = await _classRepository.GetListAsync(
                    filter: s =>
                        (!request.SubmissionId.HasValue || s.SubmissionId == request.SubmissionId.Value)
                );
